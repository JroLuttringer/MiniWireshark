...
int main(int argc, char** argv) {
  ...
  if (file_in == NULL) {
    ...
    pcap_t* dev_fd =
        pcap_open_live(dev, MAX_BYTE, PROMISC_MODE, 0, error_buffer);
    if (!dev_fd) {
  ...
  if (!filter) {
    ...
    if (pcap_compile(dev_fd, filter_bpf, filtre, 0, subnet_mask) == -1 ||
        pcap_setfilter(dev_fd, filter_bpf)) {
...
/*	pcap_if_t* dev = chose_dev();
        printf(" Interface : %s\n", dev->name);
        pcap_t* dev_fd = pcap_open_live(dev->name, MAX_BYTE, PROMISC_MODE, 0,
   error_buffer);
        //TODO: Exit si erreur?
        bpf_u_int32 * ip_dev = NULL;
        bpf_u_int32 * subnet_mask = NULL;
